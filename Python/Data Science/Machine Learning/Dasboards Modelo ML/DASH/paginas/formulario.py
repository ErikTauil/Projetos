# Criando o Formulario na Pagina Web
# pip install dash-bootstrap-components

# Fluxo e ordem do APP:
#  1 - modelo = joblib.load(...)  # modelos carregados
#  2 - formulario = dbc.Container(...)  # componentes definidos
#  3 - app.layout = html.Div([...])     # layout definido
#  4 - @app.callback(...)               # callback definido (usa os componentes acima)
#  5 - def prever_doenca(...):         # função da previsão
#  6 - app.run(debug=True)             # roda o app
...
#Apenas definir o formulário layout
...
# O @app.callback deve estar depois que os componentes que ele usa (como id='idade', id='botao-prever')...
# Isso acontece porque O Dash precisa saber quais componentes existem antes de associá-los ao callback.
# Se o @app.callback for definido antes do layout ou dos componentes, pode dar erro.
...
def prever_doenca(n_clicks, idade, sexo, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal):
    ...
    # Criando DataFrame com os dados fornecidos
    # O DataFrame é criado dentro da função prever_doenca, ou seja, somente após o clique no botão 
    # e com os dados vindos dos componentes do formulário.
    ...
    # Caso o usuário não preencha algum dos campos do formulário essa função faz a trativa de preenchimento para não da erro, ou seja, 
    # preenche os campos em branco com as medianas adicionando a ele um .fillna(medianas, inplace=True).
    # Preencher valores ausentes com as medianas do treina_modelo.py 
    # Preenche os valores ausentes (NaN) no DataFrame entradas_usuario com os valores médios ou medianos salvos no objeto medianas.
    ...
    #if previsao == 1:
    #    return html.H2("Você tem doença cardíaca", className='text-danger mt-4')
    #else:
    #    return html.H2("Você não tem doença cardíaca", className='text-success mt-4')
